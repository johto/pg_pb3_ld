						...
						ReorderBufferTupleBuf *tuple)
{
	...
	for (natt = 0; natt < tupdesc->natts; natt++)
	{
		...
		fsd_add_attribute(fsd, relation, NameStr(attr->attname), typid,
						  valdatum, isnull, EXTERNAL_ONDISK_NOTOK);
	}
}
...
fsd_serialize(PB3LD_FieldSetDescription *fsd, int32 field_number, StringInfo out)
{
	...
	if (msglen > 104857600)
		elog(ERROR, "unexpected message length %zd", msglen);

	pb3_append_varlen_key(privdata->message_buf, field_number);
	pb3_append_int32(privdata->message_buf, (int32) msglen);
	msg_start = (int64) privdata->message_buf->len;
